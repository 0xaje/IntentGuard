...
| 13 | **“What is the exact role of Orion and the autonomous AI agent in IntentGuard?”** | “The autonomous agent’s responsibility is solely to **produce a proposed action**. The agent does not evaluate its own safety, hold user private keys, or execute custody. IntentGuard sits as the independent deterministic verification gate that proves whether that proposed action matches the human user’s declared intent. The agent proposes; IntentGuard verifies.” | Architecture diagram showing Orion agent boundary; Stage 03 UI panel. |
| 14 | **“Is `MATCH` equivalent to safe?”** | “No. `MATCH` means the supplied request matched the supplied policy under the checks implemented by this engine version. It does not prove that the target contract is honest or bug-free, and it does not protect a user from signing elsewhere.” | UI limitation copy and receipt limitations. |
| 15 | **“What would you build next if you won?”** | “First, integrate verified-ABI nested-call decoding and read-only simulation. Second, add a wallet or smart-account adapter that can enforce the receipt policy in an explicit programmable account. Third, expose the receipt and policy interfaces as an Orion Agent Store service.” | Roadmap slide or closing answer. |
