...
> *IntentGuard is an intent-fidelity and cryptographic attestation layer for autonomous AI agents on Base. It converts human instructions into structured, deterministic constraints and evaluates agent-proposed transactions against observable blockchain evidence.*

**Core Thesis**:
> **"Agents can decide. IntentGuard verifies."**
> *The agent can be intelligent. The verifier doesn't have to be.*
>
> The LLM may help interpret human intent, but it does **not** decide whether the transaction is safe. IntentGuard separates interpretation from enforcement: natural language may be parsed into structured intent, but the final `MATCH` / `MISMATCH` / `CANNOT_VERIFY` verdict is produced by deterministic rules over observable transaction evidence.
>
> Pre-execution (*Can this proposed transaction safely proceed?*) and Post-execution (*Did the transaction that actually executed remain consistent?*) are distinct security boundaries. IntentGuard's current live verifier proves intent fidelity from observable Base evidence, while sharing the same deterministic policy engine designed to sit in the pre-execution path before an agent's transaction is signed.
...
| **Solidity Architecture** | Security-oriented Solidity 0.8.24 implementation with role separation, replay protection, revocation, and emergency controls (OpenZeppelin v5.0.2 + EIP-712) |
| **Test Suite Results** | **42 Passing** (17 Vitest, 9 Hardhat with gas metrics, 16 TSX Engine & Invariants) |
...
|---:|---|---|---|
| 1 | **“Do you claim full pre-execution protection?”** | “No. Pre-execution (‘Can this safely proceed?’) and post-execution (‘Did it execute as promised?’) are different security problems. Our engine verifies observable ERC-20 transfers and supported SwapRouter02 execution paths, while presenting QuoterV2 as a separate current-state estimate rather than mined execution proof. We provide the deterministic verification and EIP-712 attestation primitives that agents, wallets, and smart accounts integrate.” | README two-stage architecture and UI boundary panel. |
...
| 3 | **“You say you block transactions, but an EOA can sign elsewhere. Isn’t that misleading?”** | “Correct: the MVP does not and cannot universally stop an externally owned account from signing somewhere else. A `MISMATCH` verdict prevents continuation through an integrated agent/middleware execution path and returns a fail-closed decision. Universal enforcement requires a smart account, Safe module, wallet integration, or programmable execution layer, which we deliberately exclude from this non-custodial MVP.” | Safety-boundary slide and README limitation. |
| 4 | **“Where is the AI? This sounds like deterministic code.”** | “That separation is our core architectural guarantee: **‘The model may help interpret intent. It does not decide whether the transaction is safe.’** The LLM only parses natural language into a Zod-validated `IntentSpec`. The final safety verdict (`MATCH` / `MISMATCH` / `CANNOT_VERIFY`) is computed deterministically from typed policy constraints, decoded transaction data, and observable blockchain evidence. An LLM has zero verdict authority.” | Architecture pipeline: LLM produces `IntentSpec`; Deterministic Engine computes Verdict. |
| 5 | **“What exact transactions do you support today?”** | “The verified MVP supports native transfers with value, ERC-20 approvals and supported permit evidence (`transfer`, `approve`, ERC-2612 typed `Permit`), dynamic ERC-20 token metadata resolution (`symbol`, `decimals`), and allowlisted Uniswap V3 `SwapRouter02.exactInputSingle` routes. Permit2 and arbitrary nested multi-calls are not claimed in this core package and fail closed as `CANNOT_VERIFY`.” | TypeScript decoder, dynamic token resolver, fixture tests, and supported-scope README. |
...
| 12 | **“Where did this evidence come from? How do I verify its provenance?”** | “Every piece of evidence IntentGuard verifies is anchored in our structured Provenance Layer. We output the exact Base block number, block hash, transaction hash, mined receipt status, target contract, decoder version, and canonical evidence hash. It is forensically auditable rather than an opaque analytical score.” | Forensic Provenance UI tree and `provenance` payload. |
| 13 | **“What is the exact role of Orion and the autonomous AI agent in IntentGuard?”** | “The autonomous agent’s responsibility is solely to **propose transactions**. The agent does not evaluate its own safety, hold user private keys, or execute custody. IntentGuard provides an Orion-compatible integration prototype: `Orion Agent proposes -> IntentGuard.verify(...) -> MATCH / MISMATCH / CANNOT_VERIFY -> only MATCH continues`. The agent proposes; IntentGuard verifies.” | `examples/orion-integration.ts`, Architecture diagram, and Stage 03 UI panel. |
| 14 | **“Is `MATCH` equivalent to safe?”** | “No. `MATCH` means all required supported constraints are satisfied by the available evidence under the checks implemented by this engine version. It does not prove that the target contract is honest or bug-free, and it does not protect a user from signing elsewhere.” | UI limitation copy and receipt limitations. |
| 15 | **“What is on the V2 roadmap after the hackathon?”** | “Our concrete V2 roadmap priorities are: 1) Pre-execution state simulation; 2) Runtime enforcement packages for Orion and other agent frameworks; 3) Smart-account / Safe execution hooks; 4) Expanded protocol coverage with independently audited decoders (Permit2, Universal Router); 5) Cross-chain policy enforcement; 6) Persistent forensic verification history.” | V2 Roadmap specification in README and architecture. |
